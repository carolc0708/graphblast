#ifndef GRAPHBLAS_BACKEND_CUDA_BLOCK_MATRIX_OPERATIONS_HPP_
#define GRAPHBLAS_BACKEND_CUDA_BLOCK_MATRIX_OPERATIONS_HPP_
...
Info vxm(Vector<W>*       w,
  ...
  u_t->sparse2dense(op.identity(), desc); // <--- I added
  Storage u_vec_type;
  CHECK(u->getStorage(&u_vec_type));
  if (desc->debug())
    std::cout << "u_vec_type: " << u_vec_type << std::endl; // should be 2
  ...
  if (desc->debug())
    std::cout << "w_vec_type: " << w_vec_type << std::endl; // should be 2

  CHECK(spmv(&w->dense_, mask, accum, op, &A->block_matrix_32_, &u->dense_, desc));
...
#endif  // GRAPHBLAS_BACKEND_CUDA_BLOCK_MATRIX_OPERATIONS_HPP_
